...
import { normalizeCmsProjectList } from '$lib/contracts/cms';
...
export const GET: RequestHandler = async (event) => {
  try {
    ...
    const data = normalizeCmsProjectList(await cmsRequest(event, '/v1/cms/projects'));
    if (!data) throw new CmsClientError(502, 'backend_unavailable', 'Invalid projects payload.');
