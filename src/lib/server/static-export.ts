...
export function createStaticZip(project: BuilderProject): Uint8Array {
  ...
  contract.pages.forEach((page) => {
    ...
    const headerCta = contract.project.headerCta ? `<a class="header-cta" href="${escape(contract.project.headerCta.href)}">${escape(contract.project.headerCta.label)}</a>` : '';
    const footerLinks = contract.project.footerLinks.length ? `<nav class="footer-links">${contract.project.footerLinks.map((link) => `<a href="${escape(link.href)}">${escape(link.label)}</a>`).join('')}</nav>` : '';
    ...
    const lottieScript = usesLottie(contract)
      ...
      : '';
    files[file] = strToU8(`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(page.seo.title)}</title><meta name="description" content="${escape(page.seo.description)}">${page.seo.noIndex ? '<meta name="robots" content="noindex,nofollow">' : ''}${page.seo.image ? `<meta property="og:image" content="${escape(prefix + page.seo.image.replace(/^\//, ''))}">` : ''}<link rel="stylesheet" href="${prefix}styles.css"></head><body><header><div class="header-main"><a class="brand" href="${prefix}index.html"><span>S</span><strong>${escape(contract.project.headerTitle)}</strong></a><div class="header-actions"><nav>${navigation}</nav>${headerCta}</div></div></header><main>${body}</main><footer><div class="footer-main"><strong>${escape(contract.project.name)}</strong><span>${escape(contract.project.footerText)}</span></div>${footerLinks}</footer>${lottieScript}</body></html>`);
  });
...
export function collectMedia(project: BuilderProject): Map<string, string> {
  ...
  const regex = /\/api\/builder\/media\/([a-f0-9-]+)/gi;
...
function renderStaticBlock(block: RenderBlock): string {
  ...
  if (block.type === 'image') return wrap(`${block.src ? `<img src="${escape(block.src)}"${block.srcSet ? ` srcset="${escape(block.srcSet)}"` : ''}${block.sizes ? ` sizes="${escape(block.sizes)}"` : ''} alt="${escape(block.alt)}" style="object-position:${block.focalX}% ${block.focalY}%">` : '<div class="image-placeholder">URL gambar harus menggunakan HTTPS</div>'}<small>${escape(block.caption)}</small>`);
  ...
  if (block.type === 'grid') return wrap(`<h2>${escape(block.title)}</h2><p>${escape(block.body)}</p><div class="card-grid" style="--grid-mobile:${block.columnsMobile};--grid-tablet:${block.columnsTablet};--grid-desktop:${block.columnsDesktop}">${block.items.map((item) => `<article><strong>${escape(item.title)}</strong><p>${escape(item.body)}</p>${item.href ? `<a class="grid-link" href="${escape(item.href)}">${escape(item.button)}</a>` : ''}</article>`).join('')}</div>`);
