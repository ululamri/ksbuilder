...
export type RenderBlock =
  ...
  | (BaseRenderBlock & { type: 'form'; projectId: string; pageId: string; formId: string; title: string; body: string; button: string; action: string; fields: FormField[] });
...
export function createSiteRenderContract(project: BuilderProject, options: CreateSiteRenderContractOptions = {}): SiteRenderContract {
  ...
  return {
    ...
    pages: pages.map((page, index) => ({
      ...
      blocks: page.blocks.map((block) => toRenderBlock(block, { resolveAssetUrl, projectId: project.id, pageId: page.id }))
    }))
  };
}
...
function toRenderBlock(block: BuilderBlock, context: { resolveAssetUrl: (value: string) => string; projectId: string; pageId: string }): RenderBlock {
